...
defmodule RobotSimulatorTest do
  ...
  test "simulate robots directions starting north" do
    robot1 = RobotSimulator.create(:north, {0, 0})
            |> RobotSimulator.simulate("LAAARALA")
    assert RobotSimulator.direction(robot1) == :west
  end

  test "simulate robots directions starting east" do
    robot2 = RobotSimulator.create(:east, {2, -7})
      |> RobotSimulator.simulate("RRAAAAALA")
    assert RobotSimulator.direction(robot2) == :south
  end

  test "simulate robots directions starting south" do
    robot3 = RobotSimulator.create(:south, {8, 4})
    |> RobotSimulator.simulate("LAAARRRALLLL")
    assert RobotSimulator.direction(robot3) == :north
  end

  test "simulate robots position with default" do
    robot1 = RobotSimulator.create(:north, {0, 0})
            |> RobotSimulator.simulate("LAAARALA")
    assert RobotSimulator.position(robot1) == {-4, 1}
  end

  test "simulate robots position with positive starting point facing north" do
    robot1 = RobotSimulator.create(:north, {7, 3})
            |> RobotSimulator.simulate("RAALAL")
    assert RobotSimulator.position(robot1) == {9, 4}
  end

  test "simulate robots position with negative y" do
    robot2 = RobotSimulator.create(:east, {2, -7})
      |> RobotSimulator.simulate("RRAAAAALA")
    assert RobotSimulator.position(robot2) == {-3, -8}
  end

  test "simulate robots position with positive starting point" do
    robot3 = RobotSimulator.create(:south, {8, 4})
      |> RobotSimulator.simulate("LAAARRRALLLL")
